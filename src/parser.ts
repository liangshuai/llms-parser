...
export async function parseLlmsFromSource(source: string, options: ParseOptions = {}): Promise<LlmsDocument> {
  ...
  const raw = await loadSource(resolved, options);
...
async function loadSource(resolved: ResolvedSource, options: ParseOptions): Promise<string> {
  if (resolved.context.kind === 'url') {
    const fetcher = options.fetchImpl ?? fetch;
    try {
      const response = await fetcher(resolved.target, options.requestInit);
      if (!response.ok) {
      ...
      throw new Error(`Unable to load remote llms.txt from ${resolved.target}: ${message}`);
    }
  }
